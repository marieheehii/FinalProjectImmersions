...
public class InventoryService : IInventoryService
{
    ...
    {
        var items = await _context.Items.Select(entity => new InventoryListItem
        {
            ID = entity.ID,
    ...
    public async Task<KitchenInventoryModel> GetInventoryByIdAsync(int id)
    {
        var item = await _context.Items.FirstOrDefaultAsync(entity => entity.ID == id);
        if(item is null)
        {
            return null;
        }
        return new KitchenInventoryModel
        {
            ID = item.ID,
